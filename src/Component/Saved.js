...
const Saved = () => {
    ...
    const retrieveData=()=>{
       if(localStorage["datas"]&& JSON.parse(localStorage["datas"]).length>0)
       {
           var data = JSON.parse(localStorage["datas"])
           setIsLoaded(true);
           setPageCount(Math.ceil(data.length/perPage));
           if(isNaN(perPage)|| perPage>10){
            setStoredDatas(data.slice(offset, offset+5));
           }else{
            setStoredDatas(data.slice(offset, offset+perPage));
           }
        }else{
            setStoredDatas([]);
            setError("No Data is saved yet")
        }
    }
    ...
    const handleUnSave = (item) => {
        for( var i = 0; i < storedDatas.length; i++){ 
            ...
            if ( storedDatas[i].uuid === item.uuid) { 
        
                storedDatas.splice(i, 1); 
            }
                        ...
                        <li key={item.uuid}>
                            <article className="card" key={item.callingCodes}>
                                ...
                                <div className="card-content">
                                    ...
                                    <div className="search-wrapper">
                                    <button onClick={() => handleUnSave(item)}>Remove From Saved</button>
